...
`hops doctor --check-overlay` を実行し、続けて `hops feedback route --record <id>` を使う。`.harnessops/`、`harness-feedback/`、`harness-lab/` の構造を直接組み替えない。プロジェクト発展は `harness-feedback/` ではなく `research/` または `notes/` に置く。
