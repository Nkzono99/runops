...
この skill は `hops lab memory compact` を置き換えるものではない。`hops lab memory compact` は deterministic knowledge snapshot、つまり source ID、score、guard、open question へ戻るための索引として扱う。抽象化、統合、反例整理、評価則への昇格はこの skill が行う。
...
3. ユーザーが手動実行を依頼した場合、または lint が `needs-abstraction` の場合は `hops lab memory compact --force` で deterministic snapshot を更新し、続けて `hops lab memory prepare --force` で入力 bundle を作る。
