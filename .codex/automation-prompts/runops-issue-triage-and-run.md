...
1. If `hops` is available, run:
   `hops doctor --check-overlay --check-records`
   and `hops migrate --check`.
2. If `hops` is unavailable, continue GitHub issue triage and report
   `hops unavailable`; do not hand-edit `.harnessops/` or `harness-lab/records/`.
3. For valid recurring harness or workflow improvement issues, use
