"""Source and profile subcommands for knowledge CLI."""
...
def sync(
    ...
    if source_name:
        for source in selected_sources:
            try:
                ...
                typer.echo(
                    f"  [{source.kind}/{source.source_type}] {source.name}: {status}"
                )
            except KnowledgeSourceError as exc:
                typer.echo(
                    f"  [{source.kind}/{source.source_type}] {source.name}: "
                    f"error - {exc}"
                )
    else:
    ...
    if any(source.kind == "profiles" for source in config.sources):
        ...
        try:
            ...
            typer.echo(f"Warning: failed to render imports: {exc}", err=True)
...
def detach(
    ...
    if config:
        ...
        for source in config.sources:
            if source.name != name or not source.mount:
                continue
            candidate = root / source.mount
            try:
                ...
                mount_path = None
                break
            resolved_mount = resolved_parent / candidate.name
...
def status_cmd() -> None:
    ...
    if config is None or not any(
        source.kind == "profiles" for source in config.sources
    ):
        return
