...
7. survey がある場合は `runo analyze collect <survey>` を実行し、
   `summary/survey_summary.csv` と `summary/figures_index.json` を確認する。
8. 必要なら `runo analyze plot <survey> --list-columns` で plot 可能な列を確認する。
9. 結果と evidence path を `{{ skill_prefix }}note` に残す。
10. 研究判断が変わる場合だけ `{{ skill_prefix }}research-agenda` も更新する。
