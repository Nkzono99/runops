...
runops v1.3.0 ~/~ Claude Code (Anthropic)

\vspace{4cm}

{\small\color{gray} 2026-09-01}
\end{center}
...
\begin{itemize}[nosep]
  ...
  \item \textbf{Bounded Experiment:} 問い・baseline・有限 budget・終了条件を Experiment として admission し、AI が作る候補数と実体化する Run 数を分離
  \item \textbf{Lazy パラメータサーベイ:} survey.toml の候補は read-only に preview し、plan hash と明示選択を通った点だけを Run に materialize
  \item \textbf{TestAttempt の分離:} smoke/debug は通常 Run と別の T ID・receipt に置き、同一条件の成功済み cache hit では新しい directory を作らない
  \item \textbf{直交する整理軸:} Run の lifecycle、理由付き review、storage の hot/cold・full/compacted、Result evidence を混同しない
  ...
  \item \textbf{知見管理:} 現在判断・時系列 journal・claim ごとの Result evidence を分け、残す解析だけを seal して検証可能にする
  \item \textbf{来歴 (Provenance):} manifest.toml に run の全情報を記録し、「いつ・誰が・どの設定で・どの commit から」実行したか追跡可能
\end{itemize}

\subsection*{\color{accent} 基本概念: 有限な問いから選択済み Run へ}

runops はシミュレーション研究を次の単位で整理します。Case は再利用可能な入力テンプレート、
Experiment は今判断する一つの問いです。
...
Experiment & experiments/E...toml & 一つの問い・baseline・budget・終了条件 \\
Case & case.toml & シミュレーション設定のテンプレート \\
Survey & survey.toml & Experiment に属する lazy 候補定義 (掃引軸) \\
Run & manifest.toml & 明示 materialize した個々のシミュレーション実行 \\
\bottomrule
\end{tabular}
\end{center}

例えば「イオン温度を 1--19\,eV で 10 点掃引」という案は、まず Experiment と Survey の
read-only plan になります。この時点では Run directory も run\_id も作りません。人が選んだ
pilot 点だけを plan hash 付きで materialize すると、初めて各 Run に固有の run\_id、入力、
ジョブスクリプト、manifest が配置されます。

\begin{infobox}
これらの単位を理解しておけば、実際のファイル操作やコマンド実行は AI Agent が行います。
ユーザーは「Ti を 1--19\,eV で 10 点の候補にし、まず両端と中央だけ pilot にして」と
伝えます。AI は候補を preview し、budget と選択点を提示します。全候補を暗黙に Run 化しません。
\end{infobox}
...
これにより runtime 用 venv 作成、プロジェクト構造 (runops.toml, simulators.toml, campaign.toml, .runops/ 等) の生成が自動で行われます。runops CLI は \texttt{uvx --from runops runo ...} で実行します。
...
> 問い・baseline・budget・終了条件を持つ Experiment を作って
> Ti=1-19eV で 10 点のサーベイ候補を preview して
> smoke は正式 Run と分け、pilot は選んだ 3 点だけ Run にして
> 完了した run を解析して
\end{lstlisting}
...
.vscode/settings.json & 生成物を隠す editor 設定 \\
\bottomrule
\end{tabular}
\end{center}
...
「候補を preview して pilot 3点だけ投入して」 & plan → 選択 Run 生成 → 投入 (承認あり) \\
「smoke test をして」 & T ID の TestAttempt を準備・記録 (通常 Run と分離) \\
「今の進捗を確認して」 & 状態確認・Slurm 同期 \\
「完了した run を解析して」 & 解析・集計 \\
「失敗した run を調べて」 & 失敗 run の診断 \\
「この claim を根拠付きで残して」 & Result 作成 → Run/artifact evidence を seal \\
「今の判断を記録して」 & CURRENT または bounded journal に記録 \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{infobox}
\textbf{具体例:}\\
「Ti=1--19eV で 10 点を候補化し、両端と中央を pilot として gr20001a に投入して」\\
→ AI が Goal (投入)、Done (job ID)、Budget (run 数・資源量) を設定し、\\
Experiment 確認 → read-only plan → 3点の明示 materialize → 投入提案
(ユーザー承認) → job ID 報告まで実行する。\\
初動も Goal に含める場合は、step の進行と観測期限を Done / Budget に加える。
\end{infobox}
...
\begin{itemize}[nosep]
  ...
  \item Survey の\textbf{全点 materialize} (\texttt{--all}) と Experiment の \texttt{decision=expand}
  \item 大規模 survey の\textbf{一括投入} (\texttt{runs submit --all})
...
研究の方向性を AI Agent に自然言語で伝えます。長期目的は campaign.toml、今回判断する
一つの問いは baseline・budget・有効期限・exit criteria を持つ Experiment にします。
...
AIは依頼された到達状態までケース作成とサーベイ設計を進めます。\texttt{runo runs sweep} の
既定は read-only plan で、directory と Run ID は増えません。Run 生成には \texttt{--apply}、
\texttt{--point} または \texttt{--all}、preview の \texttt{--expect-plan} が必要です。

\begin{lstlisting}[language={},commentstyle={}]
> Ti サーベイを設計して候補を preview し、pilot 3点だけ Run にして
\end{lstlisting}

\subsection*{\color{accent} Phase 3: 実行 (AI 提案 → 人間承認)}

ジョブ投入は AI が提案し、\textbf{ユーザーが承認}してから実行されます。Agent は
Goal / Done / Budget / Invariantに沿って最短の状態遷移を選び、投入Goalはjob ID、初動確認Goalは
progress evidence を報告して完了します。

\begin{lstlisting}[language={},commentstyle={}]
> 選択済み pilot 3 点を gr20001a に投入して
> [AI が投入内容を提示 -> ユーザーが承認]
> 進捗を確認して
\end{lstlisting}
...
\begin{lstlisting}[language={},commentstyle={}]
> terminal Run を理由付きで review し、この claim を Result evidence として seal して
> 次のサーベイの方針を議論しよう
\end{lstlisting}

AI は terminal Run を理由付きで review し、残す claim だけを Result にまとめます。Result の
Run / artifact evidence は claim-local で、smoke/debug の T ID は scientific evidence に
できません。README と evidence を seal した後、Experiment の判断を更新して Phase 1 に戻ります。
...
\begin{enumerate}[nosep]
  ...
  \item 投げた issue の URL を必要に応じて \textbf{bounded journal に記録}
  \item 当面の研究作業は \textbf{workaround で継続} (フィードバックで作業を止めない)
\end{enumerate}

\begin{infobox}
\textbf{実例:} 実運用では encoding 修正、\texttt{--qos} オプション追加、
\texttt{runs sync} の集約表示などが改善提案として upstream へ還元されています。
\end{infobox}
...
\begin{lstlisting}[language={},commentstyle={}]
> /update-runops
AI: uvx 経由の runops で update-harness を実行します...
...
実行内容:
\begin{itemize}[nosep]
  \item \textbf{uvx 経由の runops} で最新 package を実行
  \item \textbf{runo update-harness} でハーネスファイル (CLAUDE.md, skills, rules, editor settings 等) を再生成
  \item \textbf{不足している visible workspace} として \texttt{materials/} と最小 \texttt{research/} を補完
...
\subsection*{\color{accent} Research journal は量で区切る}

AI Agent に残すべき作業経緯は \texttt{research/journal/active.md} へ追記します。日付ごとのファイルは増やさず、設定文字数に達したときだけ原文のまま \texttt{journal/archive/JNNNN.md} へ rotation します。現在有効な判断は \texttt{research/CURRENT.md}、残す解析は \texttt{research/results/} に分けます。

case / survey の \texttt{notes.md} と Run の \texttt{analysis/notes.md} は新規作成しません。
途中の prose は gitignored な \texttt{.runops/work/} に置き、\texttt{runo lint} の warning を
使って旧来の分散 note を見つけます。

\subsection*{\color{accent} Smoke test を活用する}

smoke/debug は \texttt{runo test smoke|debug} で T ID の TestAttempt として準備し、通常の
Run 一覧・Experiment budget・Result evidence から分離します。同一の完全な identity で直近の
成功 receipt があれば cache reuse し、新しい T ID や directory を作りません。

大規模サーベイの前に科学的な傾向を見る 2--3 点は smoke と呼ばず、正式な pilot Run として
選択 materialize します。smoke は実行系の健全性確認、pilot は研究上の evidence です。
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{runops 初期化} --- \texttt{uvx --from runops runo init \&\& uvx --from runops runo doctor}
...
\begin{itemize}[nosep]
  ...
  \item ドキュメント: \texttt{.runops/knowledge/runops/} (プロジェクト内)
\end{itemize}
