...
→ AI がサーベイ設計 → run 生成 → 投入提案 (ユーザー承認) → job ID 報告まで実行。\\
初動確認は「正常に動いているか数 step 見て」のように明示した場合だけ期限付きで行う。
\end{infobox}
...
ジョブ投入は AI が提案し、\textbf{ユーザーが承認}してから実行されます。通常は job ID を
報告して返り、投入後の状態監視は明示的に依頼した場合だけ行います。
