...
**Outcome:** Added explicit CLI/MCP reverse bindings, machine-readable conformance checks, and synchronized global/per-file coverage gates across CI, release, and agent workflows.
...
- [x] **Step 3: Implement the immutable CLI catalog and validator**
...
- [x] **Step 1: Write failing MCP reverse-contract tests**
...
- [x] **Step 3: Add ToolSpec bindings and conformance checks**
...
- [x] **Step 3: Implement the evaluator and approved policy**
...
- Modify: `.claude/skills/check/SKILL.md`
...
- [x] **Step 1: Add a failing workflow/documentation consistency test**

Extend `tests/test_harness/test_development_guidance.py` with a parameterized test over `.github/workflows/ci.yml`, `.github/workflows/publish.yml`, both agent `check` skills, and both development workflow rules. Assert every file contains `--cov-report=json:coverage.json` and `python -m runops.application.operator.coverage_policy coverage.json`.
...
- [x] **Step 3: Update all quality-gate command definitions**
...
- [x] **Step 4: Run focused tests and a real coverage report**
...
```bash
git add .github/workflows/ci.yml .github/workflows/publish.yml .agents/skills/check/SKILL.md .claude/skills/check/SKILL.md .codex/rules/dev-workflow.md .claude/rules/dev-workflow.md tests/test_harness/test_development_guidance.py
...
- [x] **Step 2: Run the complete test and coverage gates**
...
- [x] **Step 3: Run interface smoke checks**
...
- [x] **Step 5: Commit any verified closeout correction**
